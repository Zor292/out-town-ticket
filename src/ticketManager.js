...
async function handleLockConfirm(interaction) {
  ...
  const ratingAdminId = ticket.admin_id || interaction.user.id;
  const ratingAdmin   = await interaction.client.users.fetch(ratingAdminId).catch(() => interaction.user);

  await sendRatingEmbed(interaction.channel, ticket, ratingAdmin, 'ticket');

  try {
    const opener = await interaction.client.users.fetch(ticket.opener_id);
    await sendRatingEmbed(null, ticket, ratingAdmin, 'dm', opener);
  } catch (_) {}
